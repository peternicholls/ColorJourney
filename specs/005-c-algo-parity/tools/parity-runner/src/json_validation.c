...
int write_run_report(const char *artifacts_root,
    ...
    if (!artifacts_root || !summary) {
        set_error(error, "artifacts path and summary are required");
        return -1;
    }
    ...
    if (!file) {
        set_error(error, "failed to open report for writing");
        return -1;
    }
    fprintf(file, "{\n");
    fprintf(file, "  \"runId\": \"%s\",\n", provenance && provenance->run_id ? provenance->run_id : "local-run");
    fprintf(file, "  \"corpusVersion\": \"%s\",\n", provenance && provenance->corpus_version ? provenance->corpus_version : "unknown");
    fprintf(file, "  \"summary\": {\n");
    fprintf(file, "    \"totalCases\": %zu,\n", summary->total_cases);
    fprintf(file, "    \"passed\": %zu,\n", summary->passed);
    fprintf(file, "    \"failed\": %zu,\n", summary->failed);
    fprintf(file, "    \"durationMs\": %.3f\n", summary->duration_ms);
    fprintf(file, "  },\n");
    fprintf(file, "  \"provenance\": {\n");
    fprintf(file, "    \"cCommit\": \"%s\",\n", provenance && provenance->c_commit ? provenance->c_commit : "unknown");
    fprintf(file, "    \"wasmCommit\": \"%s\",\n", provenance && provenance->wasm_commit ? provenance->wasm_commit : "unknown");
    fprintf(file, "    \"platform\": \"%s\"\n", provenance && provenance->platform ? provenance->platform : "unknown");
    fprintf(file, "  }\n");
    fprintf(file, "}\n");
    fclose(file);
    return 0;
